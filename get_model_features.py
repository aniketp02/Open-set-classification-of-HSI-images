    ...
    with torch.no_grad():
        for folder in os.listdir(os.path.join(root,"open_set")):
            ...
            for file_name in os.listdir(os.path.join(root,"open_set",str(folder))):
                ...
                squeezed_latent = []
                squeezed_latent.append(logits.squeeze())
                for layer in latent:
                    m = nn.AdaptiveAvgPool2d((1,1))
                    new_layer = torch.squeeze(m(layer))
                    squeezed_latent.append(new_layer)
                
                for sl in squeezed_latent:
                    print(sl.shape)

                save_name = file_name.split(".")[0]
                print(os.path.join(save_path,"open_set",str(folder),save_name+".npy"))
    ...
    with torch.no_grad():
        for folder in os.listdir(os.path.join(root,"train")):
            ...
            for file_name in os.listdir(os.path.join(root,"train",str(folder))):
                ...
                save_name = file_name.split(".")[0]
                print(os.path.join(save_path,"train",str(folder),save_name+".npy"))
...
def epoch_val(net,save_path,root):
    ...
    with torch.no_grad():
        for folder in os.listdir(os.path.join(root,"test")):
            ...
            for file_name in os.listdir(os.path.join(root,"test",str(folder))):

                image = Image.open(os.path.join(root,"test",str(folder),file_name)).convert("RGB")
                image = transform_test(image)
                image = torch.unsqueeze(image,0)
                image=image.cuda(non_blocking=True)
                logits, _, latent = net(image)
                ...
                save_name = file_name.split(".")[0]
                print(os.path.join(save_path,"test",str(folder),save_name+".npy"))
                
                feature = torch.cat(squeezed_latent,0)

                print(save_name)
                np.save(os.path.join(save_path,"test",str(folder),save_name+".npy"),feature.cpu().data.numpy(),allow_pickle=False)
                 
def get_args():
    parser = argparse.ArgumentParser(description='Get activation vectors')
    parser.add_argument('--dataset_dir',default="./CIFAR-10-images",type=str,help="Number of members in ensemble")
    ...
    parser.add_argument('--load_path',default="./save_models/vanilla_dhr/checkpoints/90.pth",type=str,help="Path to save the ensemble weights")
...
def main():
    ...
    epoch_anomalous(net,args.save_path,root)
